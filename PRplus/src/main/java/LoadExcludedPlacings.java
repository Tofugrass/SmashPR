...
/**
 * Servlet implementation class LoadExcludedPlacings
 */
public class LoadExcludedPlacings extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public LoadExcludedPlacings() {
        super();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(excludedPlacings.size() == 0) {
			method.alertAndRedirectError("No placings excluded\n", request, response);
			return;
		}
		String player = request.getParameter("player");
		if(player == null) {
			String returnString = "Excluded Placings\n";
			for(int i = 0; i< excludedPlacings.size(); i++){
				TournamentPlacings placing = excludedPlacings.get(i);
				returnString+= placing.getPlayer().getName()+" placed " +placing.getPlacing()+" at "+placing.getTournament()+"\n";
			}
			...
			try{
				...
				String returnString = "Excluded Placings for "+playerA.getName()+"\n";
				for(int i = 0; i< excludedPlacings.size(); i++){
					TournamentPlacings placing = excludedPlacings.get(i);
